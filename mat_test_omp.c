...
// gcc mat_test_omp.c -fopenmp -lm -o mat_test_omp
...
#pragma omp parallel for default(none) private(i) shared(a) schedule(static)
...
#pragma omp parallel for default(none) private(i) shared(a ) schedule(dynamic)
    for (i = 0; i < n_rows_a; i++) {
        for (size_t j = 0; j < n_cols_a; j++) {
...
#pragma omp parallel for default(none) private(i, j) shared(m, t) schedule(static)
...
#pragma omp parallel for default(none) private(i, j, k) shared(a, b, c) schedule(static)
    for (i = 0; i < n_rows_a; i++) {
...
#pragma omp parallel for default(none) private(i, j, k) shared(a, b, c, bt) schedule(static)
    for (i = 0; i < n_rows_a; i++) {
        for (k = 0; k < n_cols_b; k++) {
            double sum = 0.0;
            for (j = 0; j < n_cols_a; j++) {
